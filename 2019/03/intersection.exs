defmodule Plotter do
  def step({ _, 0 }, line) do
    line
  end

  def step({ "R", length }, line) do
    old_pos = line |> Enum.at(-1)
    new_positions = for pt <- 1..length do
      { Kernel.elem(old_pos, 0) + pt, Kernel.elem(old_pos, 1) }
    end

    Enum.concat(line, new_positions)
  end

  def step({ "L", length }, line) do
    old_pos = line |> Enum.at(-1)
    new_positions = for pt <- 1..length do
      { Kernel.elem(old_pos, 0) - pt, Kernel.elem(old_pos, 1) }
    end
  ...
  def step({ "U", length }, line) do
    old_pos = line |> Enum.at(-1)
    new_positions = for pt <- 1..length do
      { Kernel.elem(old_pos, 0), Kernel.elem(old_pos, 1) + pt}
    end

    Enum.concat(line, new_positions)
  end

  def step({ "D", length }, line) do
    old_pos = line |> Enum.at(-1)
    new_positions = for pt <- 1..length do
      { Kernel.elem(old_pos, 0), Kernel.elem(old_pos, 1) - pt}
    end
...
defmodule Intersection do
  def enumerate(input) do
    input
    |> String.split(",")
    |> Enum.map(&String.split(&1, "", trim: true, parts: 2))
    |> Enum.map(fn [ direction, length ] -> { direction, String.to_integer(length) } end)
    |> Enum.reduce([{ 0, 0 }], fn direction, acc -> Plotter.step(direction, acc) end)
  end

  def find_intersections(line1, line2) do
    points1 = enumerate(line1)
    points2 = enumerate(line2)

    list3 = points1 -- points2
    tl(points1 -- list3)
  end

  def find_closest_intersection(line1, line2) do
    find_intersections(line1, line2)
    |> Enum.reduce(Infinity, fn { x, y }, acc -> if abs(x) + abs(y) < acc, do: abs(x) + abs(y), else: acc end)
  end

  def find_closest_steps_intersection(line1, line2) do
    points1 = enumerate(line1)
    points2 = enumerate(line2)
    find_intersections(line1, line2)
    |> Enum.map(fn int -> { Enum.find_index(points1, fn pt -> pt == int end), Enum.find_index(points2, fn pt -> pt == int end)} end)
    |> Enum.reduce(Infinity, fn { x, y }, acc -> if abs(x) + abs(y) < acc, do: abs(x) + abs(y), else: acc end)
  end
end

IO.inspect Intersection.find_closest_intersection("R8,U5,L5,D3", "U7,R6,D4,L4")
IO.inspect Intersection.find_closest_steps_intersection("R8,U5,L5,D3", "U7,R6,D4,L4")
IO.inspect Intersection.find_closest_steps_intersection("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83")
IO.inspect Intersection.find_closest_steps_intersection(
...
IO.inspect Intersection.find_closest_intersection(
